...
export declare type AnyFunction<TArgs extends any[] = any[], TReturnType = any> = (...args: TArgs) => TReturnType;
export declare type ArrayOrSingle<T> = T | T[];
export declare type ReadonlyArrayOrSingle<T> = T | readonly T[];
...
>;
export declare type NonEmptyObject<T extends AnyRecord> = keyof T extends never ? never : T;
...
export declare type MarkRequired<T, RK extends keyof T> = T extends T ? Omit<T, RK> & Required<Pick<T, RK>> : never;
/** Mark some properties as optional, leaving others unchanged */
export declare type MarkOptional<T, K extends keyof T> = T extends T ? Omit<T, K> & Partial<Pick<T, K>> : never;
/** Mark some properties as readonly, leaving others unchanged */
export declare type MarkReadonly<T, K extends keyof T> = T extends T ? Omit<T, K> & Readonly<Pick<T, K>> : never;
/** Mark some properties as writable, leaving others unchanged */
export declare type MarkWritable<T, K extends keyof T> = T extends T ? Omit<T, K> & Writable<Pick<T, K>> : never;
...
export declare type ValueOf<T> = T extends Primitive
  ? T
  : T extends AnyArray
  ? T[number]
...
export declare type Tail<T extends AnyArray> = T extends [any, ...infer Rest] ? Rest : never;
 ...
 */
export declare type PredicateType<T extends PredicateFunction> = T extends (
  target: any,
  ...rest: any[]
) => target is infer P
  ? P
